...
export function initCart(updateUICallback) {
    ...
    if (dom.cartToggle) dom.cartToggle.addEventListener('click', () => dom.cartDrawer.style.right = '0');
    if (dom.closeCart) dom.closeCart.addEventListener('click', () => dom.cartDrawer.style.right = '-100%');
    if (dom.closeModal) dom.closeModal.addEventListener('click', () => dom.successModal.style.display = 'none');
}
